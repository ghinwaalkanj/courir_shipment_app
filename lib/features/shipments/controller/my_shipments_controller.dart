...
class MyShipmentsController extends GetxController {
  ...
  List<Shipment> getActiveShipments() {
    ...
    shipment.shipmentInfo.shipmentStatus != 7 ||
        shipment.shipmentInfo.shipmentStatus != 9|| shipment.shipmentInfo.shipmentStatus != 0
    ).toList();
  }
  ...
  List<Shipment> filterShipments(int status) {
    switch (status) {
      case 1:
        return shipments.where((shipment) => shipment.shipmentInfo.shipmentStatus == 1).toList();
      case 2:
